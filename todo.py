...
class ToDo:
    ...
    def tabs_changed(self, e):
        if e.control.selected_index == 0:
            self.results = self.de_execute('SELECT * FROM tasks')
            self.view == 'all'
    ...
    def main_page(self):
        input_task = flet.TextField(
            hint_text='Digite aqui sua próxima tarefa',
            expand=True,
        ...
        tabs = flet.Tabs(
            ...
            tabs=[
                flet.Tab(text='Todos'),
                flet.Tab(text='Em andamento'),
                flet.Tab(text='Finalizados')
            ]
        )
